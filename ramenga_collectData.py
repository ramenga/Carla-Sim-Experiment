...
SaveFolder = 'H:/CARLA_0.9.10-Pre_Win/TrainingData/Carla_TrainData'
CsvFile = '/driving_log.csv'
ImageFolder = '/IMG'
file_i = 0
filename_csv = SaveFolder+'/'+'driving_log.csv'
...
def process_img(image):
	...
	rgbImage = cv2.resize(rgbImage, (320,160), cv2.INTER_AREA)
	global file_i
	filename = SaveFolder+ImageFolder+'/'+str(file_i)+'.jpg'
	
	print(filename)
	cv2.imwrite(filename, rgbImage)
	with open(filename_csv,'a',newline='') as csvfile:
		spamwriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
		spamwriter.writerow([filename, 'Lovely Spam', 'Wonderful Spam'])

	measurements = vehicle.get_control()
	print(measurements.steer())
	#vehicle.apply_control(carla.VehicleControl(throttle=0.2,steer=steering_angle*10))
